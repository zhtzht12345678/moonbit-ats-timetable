...
test "readme smoke" {
  ...
  inspect(detect_conflicts(schedule).length() > 0, content="true")
}
~~~
...
基准 CLI 使用确定性合成运行图，计数器和校验和由程序现场生成，原生主机时钟用于记录耗时：
...
```text
types.mbt
  ├─ validation.mbt       模型不变量与结构化问题
  ├─ serialization.mbt    JSON/XML 交换边界
  ├─ conflicts.mbt        运行图几何与冲突证据
  ├─ interval_index.mbt   时间区间索引与候选查询
  ├─ resources.mbt       区间、站台、咽喉和进路资源压力
  ├─ topology.mbt         有界路径和临时进路规划
  ├─ monitor.mbt          实时状态、告警和快照
  ├─ operations.mbt       客流、停站和运营报告
  ├─ service_quality.mbt  正点率、负荷和服务质量
  ├─ disruptions.mbt     封锁、限速、取消和数据中断场景
  ├─ strategy.mbt         约束过滤和调度建议
  ├─ dispatch_planner.mbt 滚动计划与策略重评分
  ├─ recovery.mbt         扣车、跳停、提速和折返恢复候选
  ├─ analytics.mbt        延误、负载和健康指标
  ├─ planning.mbt         计划复核、时刻调整和释放门
  ├─ renderer.mbt         String Diagram 与增量渲染 DTO
  ├─ benchmark.mbt        可复现工作负载与校验和
  └─ demo.mbt             合成线路和遥测回放
```

核心时间和距离计算使用整数，避免浮点数造成边界漂移。区间检测采用半开时间窗；冲突判定按车次、区间和资源分层，便于在适配层按时间窗或资源分片。

## 基准

完整的命令、工具链、主机信息和现场输出见 [docs/benchmark.md](docs/benchmark.md)。当前本地原生运行的三档工作负载为 8×24、16×72 和 24×144（站点×车次），每档输出冲突观测、区间查询、资源重叠、客流观测和确定性校验和。

## 测试

测试包括模型非法输入、时间边界、半开区间、反向查询窗口、拓扑不可达、资源窗口、场景影响、陈旧遥测、容量钳制、序列化失败和基准可复现性。开发机验证：
...
`.github/workflows/check.yml` 覆盖 Linux、macOS 和 Windows，安装稳定版 MoonBit，执行依赖更新、全目标 `moon check`、全目标 `moon test`、原生构建、格式检查、接口信息生成和工作区差异检查。当前稳定工具链的 `moon fmt` 与 `moon info` 不接受 `--deny-warn`；严格告警由 `moon check --deny-warn` 与 `moon test --deny-warn` 门禁完成，再执行官方支持的 `fmt --check` 与 `info`。

## 输入边界与来源

XML 接口是本项目定义的轻量交换子集，不承诺兼容任何厂商私有 ATS schema。生产接入应在边界层完成字段映射、单位确认、时区处理和设备数据鉴权。演示线路、车次、遥测和故障数据均为合成数据，不对应真实线路或运营机构。运行时依赖仅使用 MoonBit 官方核心 JSON 包。
